...
export const NestedWrapper = ({predicateData, scoreExtent}) => {
  ...
  console.log('PREDARRAYYYY',predArray)
...
export default function PredicateNav() {
  ...
  const [addPredMode, setAddPredMode] = useState(false);
  const [{editMode, predicateArray, deletedPredicates, parentToChildDict, hiddenPredicates, selectedPredicate}, dispatch] = useContext(DataContext);    
  ...
  let scoreExtent = d3.extent(filteredPredicates.map(m => m.predicate.score));
    ...
    return (
      <div className="pred-exp-nav-large" 
